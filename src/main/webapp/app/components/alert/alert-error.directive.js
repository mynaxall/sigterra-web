(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('sigterraWebApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-sigterraWebApp-error');
                var entityKey = httpResponse.headers('X-sigterraWebApp-params');
                if (errorHeader) {
