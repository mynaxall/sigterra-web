(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('sigterraApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-sigterraApp-error');
                var entityKey = httpResponse.headers('X-sigterraApp-params');
                if (errorHeader) {
